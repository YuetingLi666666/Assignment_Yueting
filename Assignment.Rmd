...
```{r}
GAStech_edges_aggregated2 <- GAStech_edges_new1 %>%
  left_join(GAStech_nodes_new, by = c("From" = "Name")) %>%
  rename(from = id) %>%
  left_join(GAStech_nodes_new, by = c("To" = "Name")) %>%
  rename(to = id) %>%
  ungroup()
GAStech_edges_aggregated2$title <- iconv(GAStech_edges_aggregated2$Subject, to = "utf-8")
GAStech_edges_aggregated2 <- GAStech_edges_aggregated2[,c(8,30,52)]
```
